...
// GUIDs may differ in brace/case across layers — normalise before comparing.
const normGuid = (s: string) => s.toUpperCase().replace(/[^0-9A-F]/g, '');
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const [
      ...
      process.env.PROJECT_IMPLEMENTATION_STATUSES_URL ? queryFeatures<Lookup>(process.env.PROJECT_IMPLEMENTATION_STATUSES_URL, token, '1=1', '*', 'sort_order ASC') : Promise.resolve([]),
      ...
      process.env.FACILITY_CATEGORIES_URL ? queryFeatures<Lookup>(process.env.FACILITY_CATEGORIES_URL, token) : Promise.resolve([]),
    ...
    // Facility types carry their category (GUID) so the client can cascade category -> type.
    const categoryByNorm = new Map(
      facilityCategories
        .filter((c) => c.globalid)
        .map((c) => [normGuid(c.globalid as string), c.globalid as string]),
    );
    const facilityTypeOptions = facilityTypes
      .filter((r) => r.globalid && r.facility_type)
      .map((r) => ({
        value: r.globalid as string,
        label: r.facility_type as string,
        category: r.facility_category
          ? categoryByNorm.get(normGuid(r.facility_category as string)) ?? (r.facility_category as string)
          : '',
      }));
    ...
    return res.status(200).json({
      ...
      facilityCategories: toOptions(facilityCategories, 'facility_category'),
